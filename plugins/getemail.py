...
class Plugin(AbstractPlugin):
    ...
    def __init__(self,con):
        self.con = con
        self.email_address = config.get('/email/email_address','2381324877@qq.com')
        self.email_password = config.get('/email/email_password','mnupufpoaclnecdh')
        self.pop_server_host = config.get('/email/pop_server_host','pop.qq.com')
    ...
    def handle(self,query,parsed):
        ...
        try:
            ...
            logger.debug("pop3----connect server success, now will check username")
        except:
            logger.error("pop3----sorry the given email server address connect time out")
            exit(1)
        try:
            ...
            logger.error("pop3----sorry the given email address seem do not exist")
            exit(1)
        try:
            # 验证邮箱密码是否正确
            ...
            logger.error("pop3----sorry the given username seem do not correct")
            exit(1)
